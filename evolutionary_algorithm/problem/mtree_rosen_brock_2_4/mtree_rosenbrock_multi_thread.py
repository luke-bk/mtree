...
def evaluate_chromosome(chromosome, complete_solution, sub_solution_index):
    # Modify your evaluation function to accept the complete solution and sub_solution_index
    temp_collab = complete_solution[:]  # Use slicing to create a copy
    temp_collab.insert(sub_solution_index, chromosome)  # Insert the chromosome into the solution at the specified index
    fitness_score = rosenbrock_mtree(temp_collab)  # Evaluate complete solution
    temp_collab.remove(chromosome)  # Remove the evaluated chromosome from the evaluated list
    return fitness_score


def parallel_evaluation(new_chromosomes, complete_solution, sub_solution_index):
    with concurrent.futures.ThreadPoolExecutor() as executor:  # Use ThreadPoolExecutor for multithreading
        fitness_scores = list(executor.map(evaluate_chromosome, new_chromosomes, [complete_solution]*len(new_chromosomes), [sub_solution_index]*len(new_chromosomes)))
...
def main(random_generator, is_minimization_task, dimension, lower_bounds, _upper_bounds, split_probability,
    ...
    while current_generation < max_generations:
        ...
        for leaf_node in binary_tree.get_leaf([]):
            ...
            # Perform parallel evaluation and assign fitness scores
            fitness_scores = parallel_evaluation(new_chromosomes, complete_solution, sub_solution_index)

            print(f"fitness scores are { fitness_scores}")

            # Assign the fitness scores to the new chromosomes
            for chromosome, fitness_score in zip(new_chromosomes, fitness_scores):
                total_evaluated += 1  # Increase number of evaluations counter
                total_evaluations_per_generation += 1
                chromosome.set_fitness(fitness_score)

            print(f"checking fitness scores {new_chromosomes[0].fitness}")
