...
def main(random_generator, is_minimization_task, dimension, split_probability, merge_threshold, population_size,
    ...
    for _ in range(population_size):
        pop.add_chromosome(Chromosome(random_generator, pop.get_name(), dimension, "bit"))

    # Set up the m-ary tree structure
    # Create a root node
    root_region = Region(0, dimension - 1)  # Lets us know which part of the solution its solutions cover
    ...
    quad_tree.population.elite = quad_tree.population.get_chromosome_with_max_fitness()
    ...
    while current_generation < max_generations:
        ...
        for leaf_node in quad_tree.get_leaf([]):
            ...
            complete_solution, sub_solution_index = Collaboration.collaborate(random_generator, quad_tree, leaf_node)
            ...
            # Check for new elite
            Elitism.update_elite_max(leaf_node, complete_solution, sub_solution_index)
    ...
    results.find_best_solution(quad_tree)
