...
def main(random_generator, chromosome_length, population_size, max_generations, crossover_rate, dom_increase_factor,
    ...
    for _ in range(population_size):
        pop[0].add_chromosome(Chromosome(random_generator, pop[0].get_name(), chromosome_length, "bit"))
    ...
    for individuals in pop[0].chromosomes:
        individuals.set_fitness(one_max.fitness_function(individuals))
        total_evaluated += 1

    print("  ")
    print(f"  Evaluated {len(pop[0].chromosomes)} individuals")
    ...
    while current_generation < max_generations:
        ...
        for subpopulation in pop:
            ...
            elite = subpopulation.get_chromosome_with_max_fitness()

            # Select the next generation individuals
            new_chromosomes = SelectionOperators.sus_selection_fast_clone(random_generator,
                                                                          subpopulation.chromosomes,
                                                                          len(subpopulation.chromosomes))
            ...
            subpopulation.chromosomes[:] = new_chromosomes

            # Elitism, add in the elitist individual
            subpopulation.chromosomes[-1] = elite

            print(f"  Evaluated {len(subpopulation.chromosomes)} individuals")

            fits = [ind.get_fitness() for ind in subpopulation.chromosomes]
            # Print the stats (max, min, mean, std) and write out to csv
            results.print_stats_short(subpopulation.chromosomes, fits)
            results.flush()  # Flush the content to the file after each generation

            length = len(subpopulation.chromosomes)
